...
1. 方式一：List结构：基于List结构模拟消息队列
   1. 命令：**BRPOP：带有阻塞的移除**；RPOP：不带阻塞
...
2. 方式二：PubSub：基于点对点消息模型
   - 发布订阅：消费者可以订阅一个或多个 channel，生产者向对应 channel 发送消息后，所有订阅者都能收到消息
...
```sql
XADD：向 Stream 中添加一条消息。这个命令会生成一个唯一的消息 ID，并将消息添加到 Stream 的末尾。
XREAD：从 Stream 中读取消息。可以指定读取的起始位置和数量，还可以通过堵塞模式等待新消息的到来。
```
